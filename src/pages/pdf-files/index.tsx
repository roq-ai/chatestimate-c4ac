...
import { useRoqClient, usePdfFileFindManyWithCount } from 'lib/roq';
...
export function PdfFileListPage(props: PdfFileListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'analysis.count', 'chat.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['file_name.contains', 'file_path.contains'],
          ...params.filters,
        },
        'pdf_file',
      ),
  ...
  const { data, error, isLoading, mutate } = usePdfFileFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.pdf_file.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'file_size', header: 'file_size', accessorKey: 'file_size' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.upload_date ? format(record?.upload_date, 'dd-MM-yyyy') : '',
    },
    { id: 'analyzed', header: 'Analyzed', accessorKey: 'analyzed' },
    { id: 'num_pages', header: 'num_pages', accessorKey: 'num_pages' },
