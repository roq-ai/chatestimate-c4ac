...
import { useRoqClient, useChatFindFirst } from 'lib/roq';
...
function ChatEditPage() {
  ...
  const { data, error, isLoading, mutate } = useChatFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ChatInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.chat.update({
        data: values as RoqTypes.chat,
        where: {
          id,
        },
      });
