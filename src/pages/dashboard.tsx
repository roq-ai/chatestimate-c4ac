...
import { useRoqPlatformUserProfiles, usePdfFileCount, useAnalysisCount, useChatCount } from 'lib/roq';

import { PdfFileListPage as ShowCaseList } from 'pages/pdf-files';

function DashboardPage() {
  const { session } = useSession();

  const pdfFileStat = usePdfFileCount();

  const analysisStat = useAnalysisCount();

  const chatStat = useChatCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Pdf Files',
      result: pdfFileStat?.data,
      isLoading: pdfFileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Analyses',
      result: analysisStat?.data,
      isLoading: analysisStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Chats',
      result: chatStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`pdf-files`} passHref legacyBehavior>
                  <Button
